...
namespace stable_fluids {
    ...
    struct DeviceBuffers {
        ...
        int32_t* cell_owner                        = nullptr;
        StableFluidsColliderDesc* colliders        = nullptr;
    };
    ...
    __host__ __device__ bool point_inside_collider(const StableFluidsColliderDesc& collider, const float x, const float y, const float z) {
        ...
        return fabsf(dx) <= collider.half_extent_x && fabsf(dy) <= collider.half_extent_y && fabsf(dz) <= collider.half_extent_z;
    }

    __host__ __device__ float collider_signed_distance(const StableFluidsColliderDesc& collider, const float x, const float y, const float z) {
        const float dx = x - collider.center_x;
        const float dy = y - collider.center_y;
        const float dz = z - collider.center_z;
        if (collider.collider_type == STABLE_FLUIDS_COLLIDER_SPHERE) {
            return sqrtf(dx * dx + dy * dy + dz * dz) - collider.radius;
        }

        const float qx      = fabsf(dx) - collider.half_extent_x;
        const float qy      = fabsf(dy) - collider.half_extent_y;
        const float qz      = fabsf(dz) - collider.half_extent_z;
        const float outside = sqrtf(fmaxf(qx, 0.0f) * fmaxf(qx, 0.0f) + fmaxf(qy, 0.0f) * fmaxf(qy, 0.0f) + fmaxf(qz, 0.0f) * fmaxf(qz, 0.0f));
        const float inside  = fminf(fmaxf(qx, fmaxf(qy, qz)), 0.0f);
        return outside + inside;
    }

    __host__ __device__ bool collider_is_no_slip(const uint32_t type) {
        return type == STABLE_FLUIDS_COLLIDER_VELOCITY_BOUNDARY_NO_SLIP;
    }

    __global__ void classify_cells_kernel(uint8_t* cell_flags, int32_t* cell_owner, const StableFluidsColliderDesc* colliders, const uint32_t collider_count, const int nx, const int ny, const int nz, const float h);
    __global__ void apply_domain_boundary_faces_kernel(
        uint8_t* u_flags, uint8_t* v_flags, uint8_t* w_flags, float* u_target, float* v_target, float* w_target, const int nx, const int ny, const int nz, StableFluidsDomainBoundaryDesc domain_boundary);
    __global__ void apply_collider_interface_faces_kernel(
        uint8_t* u_flags, uint8_t* v_flags, uint8_t* w_flags, float* u_target, float* v_target, float* w_target, const int32_t* cell_owner, const StableFluidsColliderDesc* colliders, const int nx, const int ny, const int nz, const float h);
    __global__ void apply_collider_no_slip_tangent_faces_kernel(
        uint8_t* u_flags, uint8_t* v_flags, uint8_t* w_flags, float* u_target, float* v_target, float* w_target, const int32_t* cell_owner, const StableFluidsColliderDesc* colliders, const int nx, const int ny, const int nz, const float h);
    ...
    StableFluidsResult build_boundary_atlas(ContextStorage& context) {
        ...
        const dim3 cells(static_cast<unsigned>((nx + static_cast<int>(block.x) - 1) / static_cast<int>(block.x)), static_cast<unsigned>((ny + static_cast<int>(block.y) - 1) / static_cast<int>(block.y)), static_cast<unsigned>((nz + static_cast<int>(block.z) - 1) / static_cast<int>(block.z)));
        const dim3 faces(static_cast<unsigned>((nx + 1 + static_cast<int>(block.x) - 1) / static_cast<int>(block.x)), static_cast<unsigned>((ny + 1 + static_cast<int>(block.y) - 1) / static_cast<int>(block.y)), static_cast<unsigned>((nz + 1 + static_cast<int>(block.z) - 1) / static_cast<int>(block.z)));

        if (cudaMemsetAsync(context.device.cell_flags, 0, cell_count * sizeof(uint8_t), context.stream) != cudaSuccess) return backend_failure;
        if (cudaMemsetAsync(context.device.cell_owner, 0xFF, cell_count * sizeof(int32_t), context.stream) != cudaSuccess) return backend_failure;
        if (cudaMemsetAsync(context.device.u_flags, 0, u_count * sizeof(uint8_t), context.stream) != cudaSuccess) return backend_failure;
        if (cudaMemsetAsync(context.device.v_flags, 0, v_count * sizeof(uint8_t), context.stream) != cudaSuccess) return backend_failure;
        if (cudaMemsetAsync(context.device.w_flags, 0, w_count * sizeof(uint8_t), context.stream) != cudaSuccess) return backend_failure;
        if (cudaMemsetAsync(context.device.u_target, 0, u_count * sizeof(float), context.stream) != cudaSuccess) return backend_failure;
        if (cudaMemsetAsync(context.device.v_target, 0, v_count * sizeof(float), context.stream) != cudaSuccess) return backend_failure;
        if (cudaMemsetAsync(context.device.w_target, 0, w_count * sizeof(float), context.stream) != cudaSuccess) return backend_failure;

        if (context.collider_count > 0) classify_cells_kernel<<<cells, block, 0, context.stream>>>(context.device.cell_flags, context.device.cell_owner, context.device.colliders, context.collider_count, nx, ny, nz, h);
        apply_domain_boundary_faces_kernel<<<faces, block, 0, context.stream>>>(context.device.u_flags, context.device.v_flags, context.device.w_flags, context.device.u_target, context.device.v_target, context.device.w_target, nx, ny, nz, context.config.domain_boundary);
        if (context.collider_count > 0) {
            apply_collider_interface_faces_kernel<<<faces, block, 0, context.stream>>>(context.device.u_flags, context.device.v_flags, context.device.w_flags, context.device.u_target, context.device.v_target, context.device.w_target, context.device.cell_owner, context.device.colliders, nx, ny, nz, h);
            apply_collider_no_slip_tangent_faces_kernel<<<faces, block, 0, context.stream>>>(context.device.u_flags, context.device.v_flags, context.device.w_flags, context.device.u_target, context.device.v_target, context.device.w_target, context.device.cell_owner, context.device.colliders, nx, ny, nz, h);
        }
    ...
    void destroy_buffers(ContextStorage& context) {
        ...
        if (context.device.cell_owner != nullptr) cudaFree(context.device.cell_owner);
        if (context.device.colliders != nullptr) cudaFree(context.device.colliders);
        ...
        context.device.cell_owner         = nullptr;
        context.device.colliders          = nullptr;
    ...
    StableFluidsResult allocate_buffers(ContextStorage& context) {
        ...
        if (cell_count > 0 && cudaMalloc(reinterpret_cast<void**>(&context.device.cell_owner), cell_count * sizeof(int32_t)) != cudaSuccess) return out_of_memory;
    ...
    __global__ void classify_cells_kernel(uint8_t* cell_flags, int32_t* cell_owner, const StableFluidsColliderDesc* colliders, const uint32_t collider_count, const int nx, const int ny, const int nz, const float h) {
        ...
        int owner                     = -1;
        for (uint32_t collider_index = 0; collider_index < collider_count; ++collider_index) {
        ...
        if (owner < 0) return;
        cell_flags[index] = cell_solid;
        cell_owner[index] = owner;
    }
    ...
    __global__ void apply_domain_boundary_faces_kernel(
        ...
        if (x <= nx && y < ny && z < nz) {
            ...
            if (x == 0) {
                u_flags[index]  = domain_boundary.x_min.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_OUTFLOW ? face_outflow : face_fixed;
                u_target[index] = domain_boundary.x_min.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_INFLOW ? domain_boundary.x_min.velocity : 0.0f;
            } else if (x == nx) {
                u_flags[index]  = domain_boundary.x_max.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_OUTFLOW ? face_outflow : face_fixed;
                u_target[index] = domain_boundary.x_max.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_INFLOW ? domain_boundary.x_max.velocity : 0.0f;
            }
            if (y == 0 && domain_boundary.y_min.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_NO_SLIP) {
                u_flags[index]  = face_fixed;
                u_target[index] = 0.0f;
            }
            if (y + 1 == ny && domain_boundary.y_max.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_NO_SLIP) {
                u_flags[index]  = face_fixed;
                u_target[index] = 0.0f;
            }
            if (z == 0 && domain_boundary.z_min.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_NO_SLIP) {
                u_flags[index]  = face_fixed;
                u_target[index] = 0.0f;
            }
            if (z + 1 == nz && domain_boundary.z_max.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_NO_SLIP) {
                u_flags[index]  = face_fixed;
                u_target[index] = 0.0f;
            }
        }
        ...
        if (x < nx && y <= ny && z < nz) {
            ...
            if (y == 0) {
                v_flags[index]  = domain_boundary.y_min.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_OUTFLOW ? face_outflow : face_fixed;
                v_target[index] = domain_boundary.y_min.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_INFLOW ? domain_boundary.y_min.velocity : 0.0f;
            } else if (y == ny) {
                v_flags[index]  = domain_boundary.y_max.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_OUTFLOW ? face_outflow : face_fixed;
                v_target[index] = domain_boundary.y_max.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_INFLOW ? domain_boundary.y_max.velocity : 0.0f;
            }
            if (x == 0 && domain_boundary.x_min.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_NO_SLIP) {
                v_flags[index]  = face_fixed;
                v_target[index] = 0.0f;
            }
            if (x + 1 == nx && domain_boundary.x_max.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_NO_SLIP) {
                v_flags[index]  = face_fixed;
                v_target[index] = 0.0f;
            }
            if (z == 0 && domain_boundary.z_min.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_NO_SLIP) {
        ...
        if (x < nx && y < ny && z <= nz) {
            const auto index = index_3d(x, y, z, nx, ny);
            if (z == 0) {
                w_flags[index]  = domain_boundary.z_min.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_OUTFLOW ? face_outflow : face_fixed;
                w_target[index] = domain_boundary.z_min.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_INFLOW ? domain_boundary.z_min.velocity : 0.0f;
            } else if (z == nz) {
                w_flags[index]  = domain_boundary.z_max.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_OUTFLOW ? face_outflow : face_fixed;
                w_target[index] = domain_boundary.z_max.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_INFLOW ? domain_boundary.z_max.velocity : 0.0f;
            }
            if (x == 0 && domain_boundary.x_min.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_NO_SLIP) {
                w_flags[index]  = face_fixed;
                w_target[index] = 0.0f;
            }
            if (x + 1 == nx && domain_boundary.x_max.type == STABLE_FLUIDS_VELOCITY_BOUNDARY_NO_SLIP) {
                w_flags[index]  = face_fixed;
    ...
    __global__ void apply_collider_interface_faces_kernel(
        ...
        if (x > 0 && x < nx && y < ny && z < nz) {
            ...
            if (u_flags[face_index] == face_open) {
                const int left_owner  = cell_owner[index_3d(x - 1, y, z, nx, ny)];
                const int right_owner = cell_owner[index_3d(x, y, z, nx, ny)];
                int owner             = -1;
                ...
                if (left_owner >= 0 && right_owner >= 0) {
                    ...
                    const float py        = (static_cast<float>(y) + 0.5f) * h;
                    const float pz        = (static_cast<float>(z) + 0.5f) * h;
                    const float left_dist = collider_signed_distance(colliders[left_owner], px, py, pz);
                    const float right_dist = collider_signed_distance(colliders[right_owner], px, py, pz);
                    owner = left_dist <= right_dist ? left_owner : right_owner;
                }
                if (owner >= 0) {
                    u_flags[face_index]  = face_fixed;
                    u_target[face_index] = colliders[owner].linear_velocity_x;
                }
            }
        }
        ...
        if (x < nx && y > 0 && y < ny && z < nz) {
            ...
            if (v_flags[face_index] == face_open) {
                const int below_owner = cell_owner[index_3d(x, y - 1, z, nx, ny)];
                const int above_owner = cell_owner[index_3d(x, y, z, nx, ny)];
                int owner             = -1;
                ...
                if (below_owner >= 0 && above_owner >= 0) {
                    ...
                    const float pz         = (static_cast<float>(z) + 0.5f) * h;
                    const float below_dist = collider_signed_distance(colliders[below_owner], px, py, pz);
                    const float above_dist = collider_signed_distance(colliders[above_owner], px, py, pz);
                    owner = below_dist <= above_dist ? below_owner : above_owner;
                }
                if (owner >= 0) {
                    v_flags[face_index]  = face_fixed;
                    v_target[face_index] = colliders[owner].linear_velocity_y;
                }
            }
        }
        ...
        if (x < nx && y < ny && z > 0 && z < nz) {
            ...
            if (w_flags[face_index] == face_open) {
                const int back_owner  = cell_owner[index_3d(x, y, z - 1, nx, ny)];
                const int front_owner = cell_owner[index_3d(x, y, z, nx, ny)];
                int owner             = -1;
                ...
                if (back_owner >= 0 && front_owner >= 0) {
                    ...
                    const float back_dist = collider_signed_distance(colliders[back_owner], px, py, pz);
                    const float front_dist = collider_signed_distance(colliders[front_owner], px, py, pz);
                    owner = back_dist <= front_dist ? back_owner : front_owner;
                }
                if (owner >= 0) {
                    w_flags[face_index]  = face_fixed;
                    w_target[face_index] = colliders[owner].linear_velocity_z;
                }
            }
        }
    }
                ...
                for (int cell_z = z - 1; cell_z <= z; ++cell_z) {
                    for (int cell_y = y - 1; cell_y <= y; ++cell_y) {
                        for (int cell_x = x - 1; cell_x <= x; ++cell_x) {
                            ...
                            const int candidate = cell_owner[index_3d(cell_x, cell_y, cell_z, nx, ny)];
                            if (candidate < 0 || !collider_is_no_slip(colliders[candidate].velocity_boundary_type)) continue;
                            const float distance = collider_signed_distance(colliders[candidate], px, py, pz);
                            if (distance >= best_distance) continue;
                            best_distance = distance;
                            owner         = candidate;
                        }
    ...
    __global__ void apply_face_constraints_kernel(float* u, float* v, float* w, const uint8_t* u_flags, const uint8_t* v_flags, const uint8_t* w_flags, const float* u_target, const float* v_target, const float* w_target, const int nx, const int ny, const int nz) {
        ...
        if (x <= nx && y < ny && z < nz) {
            const auto index = index_3d(x, y, z, nx + 1, ny);
            if (u_flags[index] == face_fixed)
                u[index] = u_target[index];
            else if (u_flags[index] == face_outflow) {
                if (x == 0)
                    u[index] = cuda::std::min(u[index], 0.0f);
                else if (x == nx)
                    u[index] = cuda::std::max(u[index], 0.0f);
            }
        }
        if (x < nx && y <= ny && z < nz) {
            const auto index = index_3d(x, y, z, nx, ny + 1);
            if (v_flags[index] == face_fixed)
                v[index] = v_target[index];
            else if (v_flags[index] == face_outflow) {
                if (y == 0)
                    v[index] = cuda::std::min(v[index], 0.0f);
                else if (y == ny)
                    v[index] = cuda::std::max(v[index], 0.0f);
            }
        }
        if (x < nx && y < ny && z <= nz) {
            const auto index = index_3d(x, y, z, nx, ny);
            if (w_flags[index] == face_fixed)
                w[index] = w_target[index];
            else if (w_flags[index] == face_outflow) {
                if (z == 0)
                    w[index] = cuda::std::min(w[index], 0.0f);
                else if (z == nz)
                    w[index] = cuda::std::max(w[index], 0.0f);
            }
        }
    }
    ...
    __global__ void add_velocity_source_kernel(
        float* u, float* v, float* w, const uint8_t* u_flags, const uint8_t* v_flags, const uint8_t* w_flags, const float center_x, const float center_y, const float center_z, const float radius, const float velocity_x, const float velocity_y, const float velocity_z, const int nx, const int ny, const int nz, const float h) {
        const int x         = static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x);
        const int y         = static_cast<int>(blockIdx.y * blockDim.y + threadIdx.y);
        const int z         = static_cast<int>(blockIdx.z * blockDim.z + threadIdx.z);
    ...
    __global__ void advect_velocity_kernel(
        float* u_dst, float* v_dst, float* w_dst, const float* u_src, const float* v_src, const float* w_src, const uint8_t* u_flags, const uint8_t* v_flags, const uint8_t* w_flags, const float* u_target, const float* v_target, const float* w_target, const uint8_t* cell_flags, const int nx, const int ny, const int nz, const float h, const float dt) {
        const int x = static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x);
        const int y = static_cast<int>(blockIdx.y * blockDim.y + threadIdx.y);
        const int z = static_cast<int>(blockIdx.z * blockDim.z + threadIdx.z);
        ...
        if (x <= nx && y < ny && z < nz) {
            const auto face_index = index_3d(x, y, z, nx + 1, ny);
            if (u_flags[face_index] == face_fixed)
                u_dst[face_index] = u_target[face_index];
            else {
                const float3 pos  = make_float3(static_cast<float>(x) * h, (static_cast<float>(y) + 0.5f) * h, (static_cast<float>(z) + 0.5f) * h);
                const float3 vel  = make_float3(sample_u_field(u_src, u_flags, u_target, pos.x, pos.y, pos.z, nx, ny, nz, h), sample_v_field(v_src, v_flags, v_target, pos.x, pos.y, pos.z, nx, ny, nz, h), sample_w_field(w_src, w_flags, w_target, pos.x, pos.y, pos.z, nx, ny, nz, h));
                float3 back       = make_float3(cuda::std::clamp(pos.x - dt * vel.x, 0.0f, static_cast<float>(nx) * h), cuda::std::clamp(pos.y - dt * vel.y, 0.0f, static_cast<float>(ny) * h), cuda::std::clamp(pos.z - dt * vel.z, 0.0f, static_cast<float>(nz) * h));
                ...
                if (cell_flags[index_3d(back_ix, back_iy, back_iz, nx, ny)] == cell_solid) {
                    ...
                    for (int iteration = 0; iteration < 8; ++iteration) {
                        ...
                        if (cell_flags[index_3d(mid_ix, mid_iy, mid_iz, nx, ny)] == cell_solid)
                            hi = mid;
                        else
                            lo = mid;
                    }
        ...
        if (x < nx && y <= ny && z < nz) {
            ...
            if (v_flags[face_index] == face_fixed)
                v_dst[face_index] = v_target[face_index];
            else {
        ...
        if (x < nx && y < ny && z <= nz) {
            const auto face_index = index_3d(x, y, z, nx, ny);
            if (w_flags[face_index] == face_fixed)
                w_dst[face_index] = w_target[face_index];
            else {
    ...
    __global__ void advect_scalar_kernel(float* dst, const float* src, const float* u, const float* v, const float* w, const uint8_t* u_flags, const uint8_t* v_flags, const uint8_t* w_flags, const float* u_target, const float* v_target, const float* w_target, const uint8_t* cell_flags, const int nx, const int ny, const int nz, const float h, const float dt,
        const uint32_t extension_mode, const float constant_value) {
        const int x = static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x);
        const int y = static_cast<int>(blockIdx.y * blockDim.y + threadIdx.y);
        const int z = static_cast<int>(blockIdx.z * blockDim.z + threadIdx.z);
        if (x >= nx || y >= ny || z >= nz) return;
        ...
        const float3 vel      = make_float3(sample_u_field(u, u_flags, u_target, pos.x, pos.y, pos.z, nx, ny, nz, h), sample_v_field(v, v_flags, v_target, pos.x, pos.y, pos.z, nx, ny, nz, h), sample_w_field(w, w_flags, w_target, pos.x, pos.y, pos.z, nx, ny, nz, h));
        ...
        if (extension_mode == STABLE_FLUIDS_FIELD_EXTENSION_CONSTANT || extension_mode == STABLE_FLUIDS_FIELD_EXTENSION_STREAK) {
            back = make_float3(cuda::std::clamp(back.x, 0.0f, static_cast<float>(nx) * h), cuda::std::clamp(back.y, 0.0f, static_cast<float>(ny) * h), cuda::std::clamp(back.z, 0.0f, static_cast<float>(nz) * h));
        }
    ...
    __global__ void project_velocity_kernel(float* u, float* v, float* w, const float* pressure, const uint8_t* cell_flags, const uint8_t* u_flags, const uint8_t* v_flags, const uint8_t* w_flags, const float* u_target, const float* v_target, const float* w_target, const int nx, const int ny, const int nz, const float inv_h) {
        ...
        if (x <= nx && y < ny && z < nz) {
            const auto face_index = index_3d(x, y, z, nx + 1, ny);
            if (u_flags[face_index] == face_fixed)
                u[face_index] = u_target[face_index];
            else if (u_flags[face_index] == face_outflow) {
                if (x == 0 && cell_flags[index_3d(0, y, z, nx, ny)] != cell_solid)
                    u[face_index] -= pressure[index_3d(0, y, z, nx, ny)] * inv_h;
                else if (x == nx && cell_flags[index_3d(nx - 1, y, z, nx, ny)] != cell_solid)
                    u[face_index] += pressure[index_3d(nx - 1, y, z, nx, ny)] * inv_h;
            } else if (u_flags[face_index] == face_open && x > 0 && x < nx && cell_flags[index_3d(x - 1, y, z, nx, ny)] != cell_solid && cell_flags[index_3d(x, y, z, nx, ny)] != cell_solid)
                u[face_index] -= (pressure[index_3d(x, y, z, nx, ny)] - pressure[index_3d(x - 1, y, z, nx, ny)]) * inv_h;
        }

        if (x < nx && y <= ny && z < nz) {
            const auto face_index = index_3d(x, y, z, nx, ny + 1);
            if (v_flags[face_index] == face_fixed)
                v[face_index] = v_target[face_index];
            else if (v_flags[face_index] == face_outflow) {
                if (y == 0 && cell_flags[index_3d(x, 0, z, nx, ny)] != cell_solid)
                    v[face_index] -= pressure[index_3d(x, 0, z, nx, ny)] * inv_h;
                else if (y == ny && cell_flags[index_3d(x, ny - 1, z, nx, ny)] != cell_solid)
                    v[face_index] += pressure[index_3d(x, ny - 1, z, nx, ny)] * inv_h;
            } else if (v_flags[face_index] == face_open && y > 0 && y < ny && cell_flags[index_3d(x, y - 1, z, nx, ny)] != cell_solid && cell_flags[index_3d(x, y, z, nx, ny)] != cell_solid)
                v[face_index] -= (pressure[index_3d(x, y, z, nx, ny)] - pressure[index_3d(x, y - 1, z, nx, ny)]) * inv_h;
        }

        if (x < nx && y < ny && z <= nz) {
            const auto face_index = index_3d(x, y, z, nx, ny);
            if (w_flags[face_index] == face_fixed)
                w[face_index] = w_target[face_index];
            else if (w_flags[face_index] == face_outflow) {
                if (z == 0 && cell_flags[index_3d(x, y, 0, nx, ny)] != cell_solid)
                    w[face_index] -= pressure[index_3d(x, y, 0, nx, ny)] * inv_h;
                else if (z == nz && cell_flags[index_3d(x, y, nz - 1, nx, ny)] != cell_solid)
                    w[face_index] += pressure[index_3d(x, y, nz - 1, nx, ny)] * inv_h;
            } else if (w_flags[face_index] == face_open && z > 0 && z < nz && cell_flags[index_3d(x, y, z - 1, nx, ny)] != cell_solid && cell_flags[index_3d(x, y, z, nx, ny)] != cell_solid)
                w[face_index] -= (pressure[index_3d(x, y, z, nx, ny)] - pressure[index_3d(x, y, z - 1, nx, ny)]) * inv_h;
        }
    }
...
namespace {
    ...
    StableFluidsResult apply_velocity_constraints(stable_fluids::ContextStorage& context, const dim3& block, const dim3& faces) {
        stable_fluids::apply_face_constraints_kernel<<<faces, block, 0, context.stream>>>(
            context.device.velocity_x, context.device.velocity_y, context.device.velocity_z, context.device.u_flags, context.device.v_flags, context.device.w_flags, context.device.u_target, context.device.v_target, context.device.w_target, context.config.nx, context.config.ny, context.config.nz);
    ...
    StableFluidsResult reset_fields(stable_fluids::ContextStorage& context) {
        ...
        const dim3 faces(static_cast<unsigned>((context.config.nx + 1 + static_cast<int>(block.x) - 1) / static_cast<int>(block.x)), static_cast<unsigned>((context.config.ny + 1 + static_cast<int>(block.y) - 1) / static_cast<int>(block.y)), static_cast<unsigned>((context.config.nz + 1 + static_cast<int>(block.z) - 1) / static_cast<int>(block.z)));
        const auto cell_count = static_cast<std::uint64_t>(context.config.nx) * static_cast<std::uint64_t>(context.config.ny) * static_cast<std::uint64_t>(context.config.nz);
    ...
    StableFluidsResult project_velocity(stable_fluids::ContextStorage& context) {
        ...
        stable_fluids::project_velocity_kernel<<<faces, block, 0, context.stream>>>(
    ...
    StableFluidsResult advect_and_diffuse_scalar(stable_fluids::ContextStorage& context, const stable_fluids::FieldStorage& field) {
        ...
        for (uint32_t component = 0; component < field.desc.component_count; ++component) {
            ...
            if ((field.desc.flags & STABLE_FLUIDS_FIELD_ADVECT) != 0u) {
                stable_fluids::advect_scalar_kernel<<<cells, block, 0, context.stream>>>(temp_component, field_component, context.device.velocity_x, context.device.velocity_y, context.device.velocity_z, context.device.u_flags, context.device.v_flags, context.device.w_flags, context.device.u_target, context.device.v_target, context.device.w_target,
                    context.device.cell_flags, context.config.nx, context.config.ny, context.config.nz, context.config.cell_size, context.config.dt, field.desc.extension_mode, constant_value);
                if (cudaGetLastError() != cudaSuccess) return stable_fluids::backend_failure;
...
StableFluidsResult stable_fluids_create_context_cuda(const StableFluidsContextCreateDesc* desc, StableFluidsContext* out_context, StableFluidsFieldHandle* out_field_handles, const uint32_t out_field_handle_capacity) {
    *out_context = nullptr;
    ...
    if (desc->buoyancy_term_count > 0)
        context->buoyancy_terms.assign(desc->buoyancy_terms, desc->buoyancy_terms + desc->buoyancy_term_count);
    else
        context->buoyancy_terms.clear();
...
StableFluidsResult stable_fluids_update_scene_cuda(StableFluidsContext context, const StableFluidsSceneDesc* desc) {
    ...
    if (storage->device.colliders != nullptr) {
        cudaFree(storage->device.colliders);
        storage->device.colliders = nullptr;
    }
    storage->collider_count = desc->collider_count;
    if (storage->collider_count > 0) {
        const auto bytes = static_cast<std::size_t>(storage->collider_count) * sizeof(StableFluidsColliderDesc);
        if (cudaMalloc(reinterpret_cast<void**>(&storage->device.colliders), bytes) != cudaSuccess) return stable_fluids::out_of_memory;
        if (cudaMemcpy(storage->device.colliders, desc->colliders, bytes, cudaMemcpyHostToDevice) != cudaSuccess) return stable_fluids::backend_failure;
    }
...
StableFluidsResult stable_fluids_step_cuda(StableFluidsContext context, const StableFluidsStepDesc* desc) {
    ...
    const dim3 faces(static_cast<unsigned>((storage.config.nx + 1 + static_cast<int>(block.x) - 1) / static_cast<int>(block.x)), static_cast<unsigned>((storage.config.ny + 1 + static_cast<int>(block.y) - 1) / static_cast<int>(block.y)), static_cast<unsigned>((storage.config.nz + 1 + static_cast<int>(block.z) - 1) / static_cast<int>(block.z)));
    ...
    for (uint32_t index = 0; index < desc->velocity_source_count; ++index) {
        ...
        stable_fluids::add_velocity_source_kernel<<<faces, block, 0, storage.stream>>>(storage.device.velocity_x, storage.device.velocity_y, storage.device.velocity_z, storage.device.u_flags, storage.device.v_flags, storage.device.w_flags, source.center_x, source.center_y, source.center_z, source.radius, source.velocity_x, source.velocity_y,
            source.velocity_z, storage.config.nx, storage.config.ny, storage.config.nz, storage.config.cell_size);
        if (cudaGetLastError() != cudaSuccess) return stable_fluids::backend_failure;
    }
    ...
    for (uint32_t index = 0; index < desc->field_source_count; ++index) {
        ...
        stable_fluids::add_field_source_kernel<<<cells, block, 0, storage.stream>>>(
            field->data, static_cast<int>(field->desc.component_count), storage.device.cell_flags, source.center_x, source.center_y, source.center_z, source.radius, source.value_0, source.value_1, source.value_2, source.value_3, storage.config.nx, storage.config.ny, storage.config.nz, storage.config.cell_size);
        if (cudaGetLastError() != cudaSuccess) return stable_fluids::backend_failure;
    }

    if (const StableFluidsResult code = apply_velocity_constraints(storage, block, faces); code != stable_fluids::success) return code;

    stable_fluids::add_uniform_forces_kernel<<<faces, block, 0, storage.stream>>>(
    ...
    stable_fluids::advect_velocity_kernel<<<faces, block, 0, storage.stream>>>(storage.device.temp_velocity_x, storage.device.temp_velocity_y, storage.device.temp_velocity_z, storage.device.velocity_x, storage.device.velocity_y, storage.device.velocity_z, storage.device.u_flags, storage.device.v_flags, storage.device.w_flags, storage.device.u_target,
        storage.device.v_target, storage.device.w_target, storage.device.cell_flags, storage.config.nx, storage.config.ny, storage.config.nz, storage.config.cell_size, storage.config.dt);
    if (cudaGetLastError() != cudaSuccess) return stable_fluids::backend_failure;
...
StableFluidsResult stable_fluids_export_cuda(StableFluidsContext context, const StableFluidsExportDesc* desc, void* destination) {
    ...
    switch (desc->kind) {
    case STABLE_FLUIDS_EXPORT_FIELD_COMPONENTS:
        {
            const auto* field = &storage.fields[static_cast<std::size_t>(desc->field_a - 1u)];
    ...
    case STABLE_FLUIDS_EXPORT_ALPHA_RGB_RGBA:
        {
            const auto* alpha_field = &storage.fields[static_cast<std::size_t>(desc->field_a - 1u)];
            const auto* rgb_field   = &storage.fields[static_cast<std::size_t>(desc->field_b - 1u)];
    ...
    case STABLE_FLUIDS_EXPORT_VELOCITY_MAGNITUDE:
        ...
        return cudaGetLastError() == cudaSuccess ? stable_fluids::success : stable_fluids::backend_failure;
    case STABLE_FLUIDS_EXPORT_SOLID_MASK: stable_fluids::export_solid_mask_kernel<<<cells, block, 0, storage.stream>>>(static_cast<float*>(destination), storage.device.cell_flags, storage.config.nx, storage.config.ny, storage.config.nz); return cudaGetLastError() == cudaSuccess ? stable_fluids::success : stable_fluids::backend_failure;
