...
int32_t stable_fluids_add_force_cuda(const StableFluidsAddForceDesc* desc) {
    using namespace stable_fluids;
    if (const int32_t code = stable_fluids_validate_add_force_desc(desc); code != 0) return code;
