...
void advect_u(float* destination, const float* source_x, const float* source_y, const float* source_z, const int nx, const int ny, const int nz, const float h, const float dt, const uint32_t boundary_mask) {
    for (int z = 0; z < nz; ++z)
        for (int y = 0; y < ny; ++y)
            for (int x = 0; x <= nx; ++x)
...
void advect_v(float* destination, const float* source_x, const float* source_y, const float* source_z, const int nx, const int ny, const int nz, const float h, const float dt, const uint32_t boundary_mask) {
    for (int z = 0; z < nz; ++z)
        for (int y = 0; y <= ny; ++y)
            for (int x = 0; x < nx; ++x)
...
void advect_w(float* destination, const float* source_x, const float* source_y, const float* source_z, const int nx, const int ny, const int nz, const float h, const float dt, const uint32_t boundary_mask) {
    for (int z = 0; z <= nz; ++z)
        for (int y = 0; y < ny; ++y)
            for (int x = 0; x < nx; ++x)
...
void advect_scalar(float* destination, const float* source, const float* velocity_x, const float* velocity_y, const float* velocity_z, const int nx, const int ny, const int nz, const float h, const float dt, const uint32_t boundary_mask) {
    for (int z = 0; z < nz; ++z)
        for (int y = 0; y < ny; ++y)
            for (int x = 0; x < nx; ++x) {
                float px = (static_cast<float>(x) + 0.5f) * h;
...
void diffuse_grid(float* destination, const float* source, const int sx, const int sy, const int sz, const float alpha, const float denom, const int diffuse_iterations, const uint32_t boundary_mask) {
...
void diffuse_velocity(float* velocity_x_destination, float* velocity_y_destination, float* velocity_z_destination, const float* velocity_x_source, const float* velocity_y_source, const float* velocity_z_source, const int nx, const int ny, const int nz, const float alpha, const float denom, const int diffuse_iterations, const uint32_t boundary_mask) {
    for (int iteration = 0; iteration < diffuse_iterations; ++iteration)
        for (int parity = 0; parity < 2; ++parity) {
            for (int z = 0; z < nz; ++z)
                for (int y = 0; y < ny; ++y)
...
void compute_divergence(float* divergence, const float* velocity_x, const float* velocity_y, const float* velocity_z, const int nx, const int ny, const int nz, const float h, const uint32_t boundary_mask) {
    for (int z = 0; z < nz; ++z)
        for (int y = 0; y < ny; ++y)
            for (int x = 0; x < nx; ++x)
...
void pressure_rbgs(float* pressure, const float* divergence, const int nx, const int ny, const int nz, const int parity, const uint32_t boundary_mask) {
    for (int z = 0; z < nz; ++z)
        for (int y = 0; y < ny; ++y)
            for (int x = 0; x < nx; ++x) {
                if (((x + y + z) & 1) != parity) continue;
...
void project_u(float* velocity_x, const float* pressure, const int nx, const int ny, const int nz, const float inv_h, const uint32_t boundary_mask) {
    for (int z = 0; z < nz; ++z)
        for (int y = 0; y < ny; ++y)
            for (int x = 0; x <= nx; ++x)
                if (x == 0) {
...
void project_v(float* velocity_y, const float* pressure, const int nx, const int ny, const int nz, const float inv_h, const uint32_t boundary_mask) {
    for (int z = 0; z < nz; ++z)
        for (int y = 0; y <= ny; ++y)
            for (int x = 0; x < nx; ++x)
                if (y == 0) {
...
void project_w(float* velocity_z, const float* pressure, const int nx, const int ny, const int nz, const float inv_h, const uint32_t boundary_mask) {
    for (int z = 0; z <= nz; ++z)
        for (int y = 0; y < ny; ++y)
            for (int x = 0; x < nx; ++x)
                if (z == 0) {
...
int32_t stable_fluids_step_cpu(const StableFluidsStepDesc* desc) {
    ...
    advect_u(velocity_x_temporary, velocity_x_previous, velocity_y_previous, velocity_z_previous, nx, ny, nz, cell_size, dt, boundary_mask);
    advect_v(velocity_y_temporary, velocity_x_previous, velocity_y_previous, velocity_z_previous, nx, ny, nz, cell_size, dt, boundary_mask);
    advect_w(velocity_z_temporary, velocity_x_previous, velocity_y_previous, velocity_z_previous, nx, ny, nz, cell_size, dt, boundary_mask);
    ...
    if (viscosity <= 0.0f) {
        ...
        diffuse_velocity(velocity_x_field, velocity_y_field, velocity_z_field, velocity_x_temporary, velocity_y_temporary, velocity_z_temporary, nx, ny, nz, alpha, denom, diffuse_iterations, boundary_mask);
    }

    std::memset(pressure, 0, static_cast<std::size_t>(cell_bytes));
    compute_divergence(divergence, velocity_x_field, velocity_y_field, velocity_z_field, nx, ny, nz, cell_size, boundary_mask);
    for (int iteration = 0; iteration < pressure_iterations; ++iteration) {
        pressure_rbgs(pressure, divergence, nx, ny, nz, 0, boundary_mask);
        pressure_rbgs(pressure, divergence, nx, ny, nz, 1, boundary_mask);
    }
    project_u(velocity_x_field, pressure, nx, ny, nz, 1.0f / cell_size, boundary_mask);
    project_v(velocity_y_field, pressure, nx, ny, nz, 1.0f / cell_size, boundary_mask);
    project_w(velocity_z_field, pressure, nx, ny, nz, 1.0f / cell_size, boundary_mask);

    std::memcpy(density_previous, density_field, static_cast<std::size_t>(cell_bytes));
    advect_scalar(density_temporary, density_previous, velocity_x_field, velocity_y_field, velocity_z_field, nx, ny, nz, cell_size, dt, boundary_mask);
    ...
    if (diffusion <= 0.0f) std::memcpy(density_field, density_temporary, static_cast<std::size_t>(cell_bytes));
        ...
        const float denom = 1.0f + 6.0f * alpha;
        diffuse_grid(density_field, density_temporary, nx, ny, nz, alpha, denom, diffuse_iterations, boundary_mask);
    }
